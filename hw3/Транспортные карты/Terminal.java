...
public class Terminal {
    ...
    public void checkPayment(Card card){
        if (card instanceof Fix) {
            if (DAYS.between(((Fix) card).date, LocalDate.now()) <= 30) {
                System.out.println("\nПоездка по карте " + card.id);
                System.out.println("Проездной заканчивается " + ((Fix) card).date.plusMonths(1));
                this.trips += 1;
            } else if (card.getBalance() >= ((Fix) card).FixPayment){
                card.changeBalance(-((Fix) card).FixPayment);
                ((Fix) card).date = LocalDate.now();
                this.earned += ((Fix) card).FixPayment;
                this.trips += 1;
                System.out.println("\nПоездка по карте " + card.id);
